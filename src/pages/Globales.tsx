...
export const Globales = () => {
 ...
 const { cases } = data ?? {};
  ...
  const RECOVERED_MONTH_AGO: number = covidMonthAgo?.data?.recovered[MONTH_AGO_DATE];

  const dataYesterday: number[] = [CASES_YESTERDAY, DEATHS_YESTERDAY, RECOVERED_YESTERDAY];
  const dataMonthAgo: number[] = [CASES_MONTH_AGO, DEATHS_MONTH_AGO, RECOVERED_MONTH_AGO];

  const increaseCasesYesterday: number = ((cases - CASES_YESTERDAY)*100)/cases; 
  const increaseCasesMonthAgo: number = ((cases - CASES_MONTH_AGO) * 100) / cases; 
  console.log(cases);
  console.log(CASES_YESTERDAY,CASES_MONTH_AGO);
